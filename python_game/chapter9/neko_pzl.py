...
def check_neko(): #고양이 연결 확인 함수(판정용 리스트로 확인 후 실제 리스트에 결과 반
    for y in range(10):
        for x in range(8):
            check[y][x] = neko[y][x] #ckeck[][]에 neko[][] 데이터 복사

    for y in range(1, 9):
        for x in range(8):
            if check[y][x] > 0:
                if check[y-1][x] == check[y][x] and check[y+1][x] == check[y][x]: #가로
                    neko[y-1][x] = 7
                    neko[y][x] = 7
                    neko[y+1][x] = 7
    ...
    for y in range(10):
        for x in range(1, 7):
            if check[y][x] >0:
                 if check[y][x-1] == check[y][x] and check[y][x+1] == check[y][x]: #세로
                     neko[y][x-1] = 7
                     neko[y][x] = 7
                     neko[y][x+1] = 7
    ...
    for y in range(1, 9):
        for x in range(1, 7):
            if check[y][x] >0:
                if check[y-1][x-1] == check[y][x] and check[y+1][x+1] == check[y][x]: #대각선
                     neko[y-1][x+1] = 7
                     neko[y][x] = 7
                     neko[y+1][x+1] = 7
                if check[y+1][x-1] == check[y][x] and check[y-1][x+1] == check[y][x]: #대각선
...
def game_main():
    ...
    if index == 0:
        ...
        cvs.create_rectangle(168, 528, 456, 600, fill="skyblue", width = 0, tag = "TITLE")
        draw_txt("Normal", 312, 564, 40, "lightgreen", "TITLE")
        cvs.create_rectangle(168, 672, 456, 744, fill="skyblue", width = 0, tag = "TITLE")
        draw_txt("Hard", 312, 708, 40, "orange", "TITLE")
        index = 1
        mouse_c = 0
    elif index == 1:
        dificulty = 0
